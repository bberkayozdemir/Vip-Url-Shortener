...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
  <h1 class="h2">Urls <?php if (isset($user_data)) { echo " Of ".$user_data->name; }?></h1>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        <?php
        if (!isset($only_user)) { ?>
        $(".urltable-content").load("<?=base_url()?>admin/url_list");
        <?php } else { ?>
        $(".urltable-content").load("<?=base_url()?>admin/url_list?user=<?=$only_user?>");
        <?php } ?>
        ...
        $(".urltable-content").on("click", "a[data-action]", function(e){
            ...
            if ($(this).attr("data-action") == "view"){
                ...
                $.ajax({
                    url: '<?=base_url()?>admin/urls/' + $(this).parent().attr("data-id") + "/edit",
                    contentType: false,
                    processData: false,
                    beforeSend: function(){
                ...
                $.ajax({
                    url: '<?=base_url()?>admin/urls/' + $(this).parent().attr("data-id") + "/update-status",
                    contentType: false,
                    processData: false,
                    beforeSend: function(){
                        if (status == "1")
        ...
        $("#delete-yes").click(function(){
            $.ajax({
                url: '<?=base_url()?>admin/urls/' + $("input[name='delete-id']").attr("value") + "/delete",
                contentType: false,
                processData: false,
                success: function( data){
        ...
        $(".urltable-content").on("click", ".urltable-pagination a",function(e){
            ...
            if ($(this).attr("page") != "active")
            {
                ...
                if (typeof q !== typeof undefined && q !== false)
                    $(".urltable-content").load("<?=base_url()?>admin/url_list/" + $(this).attr("page") + "/" + $(this).attr("search"));
                else
                    $(".urltable-content").load("<?=base_url()?>admin/url_list/" + $(this).attr("page"));
            }
        });
        ...
        $("#search-url").submit(function(e){
            ...
            <?php
            if (!isset($only_user)) { ?>
            $(".urltable-content").load("<?=base_url()?>admin/url_list/1/"+$("#search-url input").val());
            <?php } else { ?>
            $(".urltable-content").load("<?=base_url()?>admin/url_list/1/"+$("#search-url input").val()+"?user=<?=$only_user?>");
            <?php } ?>
